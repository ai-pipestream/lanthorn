...
**And the guard puts `Standard` back when it drops** (SQ-0959), so a case that names a palette leaves the process on the default rather than on the last machine it booted — which is the table nextest's fresh process would have given the next case. It restores the DEFAULT, not the value it displaced: a suite that still takes the raw `V6_PALETTE_LOCK` and calls `zvm::screen::set_palette` itself restores nothing, and restore-previous would copy that machine's table forward through every later guard for ever. Twenty-eight suites are still on the raw lock, because each sets its palette deep inside its own `boot()` from a profile the case has not resolved yet; until they move, "a guard cleans up, a raw lock does not" is the whole of the rule, and a guard that runs afterwards cleans up after them.
