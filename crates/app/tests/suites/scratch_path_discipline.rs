...
//! |---|---|---|
//! | [`a_shared_helper_names_more_than_the_pid`] | a bare-pid scratch path inside a fn that is not a test | a helper is called once per CALLER, and the pid does not move between calls |
...
//!
//! The remedy for both, inside `app`, is [`app::scratch_dir`], which is unique per call
//! by construction. Crates that cannot depend on `app` — `zvm`, `gvm` and `scott` take
//! zero external dependencies — spell an `AtomicUsize` beside the pid locally. The rule
//! is the counter, not the function.
//!
//! # What this deliberately does NOT catch, and why it is still worth having
//!
//! A helper that interpolates a `tag` its callers pass satisfies the first case, and a
//! tag two callers spell the same way collides exactly as a bare pid does. That is the
//! exemplar's own shape, so the honest claim is narrow: this makes the *bare* form
//! unreachable and leaves the tag form a hand-maintained invariant. It is worth having
//! anyway because the bare form is the one someone reaches for when the helper has no
//! natural tag, and because a source check fires when the file is ADDED rather than on
//! the CI run of some unrelated commit a week later.
//!
...
/// A helper is called once per CALLER, and the pid is the same for all of them.
///
/// This is the exemplar's shape with the tag taken away, and the one a new harness
/// reaches for when its scratch directory has nothing to name it by: a free-standing
/// `fn boot()` or `fn launch()` that every case in the suite calls, building one
...
/// on the way out.
///
/// Falsified by putting a bare-pid path back into any helper it names — done for
/// SQ-1131 with `v6_macintosh_profile::launch`, whose eighteen call sites all resolved
/// to `lanthorn-mac-profile-<pid>`.
#[test]
fn a_shared_helper_names_more_than_the_pid() {
    let sites = scratch_sites();
    let bad: Vec<String> = sites
        .iter()
        .filter(|s| !s.inside_a_test && s.other_holes == 0)
        .map(|s| format!("{}:{} — \"{}\"", s.file, s.line, s.literal))
        .collect();
    assert!(
        bad.is_empty(),
        "these scratch paths are built inside a helper from the process id alone:\n\
         \x20   {}\n\
         A helper runs once per CALLER, and the pid does not change between calls: under \
         cargo test — which is what CI runs — one binary's tests share a process, so every \
         caller gets the same directory, truncates each other's fixtures, and deletes them \
         mid-read (SQ-1131). cargo nextest run cannot see this, because it gives every test \
         its own process. Inside app, take\n\
         \x20   let dir = app::scratch_dir(\"a-tag\");   // unique per call, created empty\n\
         and elsewhere put an AtomicUsize beside the pid.",
        bad.join("\n\x20   ")
    );
    assert!(
        sites.len() >= 100,
