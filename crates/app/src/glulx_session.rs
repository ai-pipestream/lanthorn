...
impl GlulxSession {
    ...
    /// banner.
    pub fn new(image: Vec<u8>, cols: u32, rows: u32, honor_game_colours: bool) -> Result<GlulxSession, GError> {
        let mem = Memory::new(image)?;
        let backend = Box::new(AppGlk::new(cols, rows, honor_game_colours));
