...
//!
//! The protocol is built per row per frame (uncached). That is functionally
//! correct but rebuilds/resizes the whole image on every visible row each frame;
//! if this proves hot, cache keyed by
