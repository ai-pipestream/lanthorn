...
const INTERPRETER: &[Row] = &[
    ...
    d(
        ...
        "\"\"",
        &[
            "Which of your own boot disks under ~/.lanthorn/ answers first when",
            "several carry the machine's system typeface. A case-insensitive piece of",
            "the disk image's filename — \"1.3\" picks the Workbench 1.3 floppy out of a",
            "drawer holding both — and empty means no preference.",
            "It only breaks a tie. Every disk of the right kind is read and the faces",
            "pool together, so a disk named here that does not carry the face being",
            "asked for falls through to the others rather than losing it; with no",
            "preference the pool is ordered by filename.",
            "Drop a Workbench floppy or a Mac OS System disk in ~/.lanthorn/ and a",
            "Version 6 game off that machine's own media is drawn with the face the",
            "machine really used — Geneva on a Macintosh, which lives in the System",
            "file and on no Infocom disk. Nothing is shipped and nothing is copied;",
            "the disks stay yours. With none there, the built-in face answers exactly",
            "as before.",
        ],
    ),
    d(
