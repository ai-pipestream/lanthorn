...
babelmap plays audio through the `audio` crate, which uses
[rodio](https://github.com/RustAudio/rodio) to open the operating system's
default audio output device **on the machine babelmap is running on**. That
...
If you SSH into a remote box and run babelmap there, its sound goes out the
*remote* machine's speakers, not yours. babelmap has no built-in
...
Point the *remote* machine (where babelmap runs) at your *local* machine's
sound server by setting `PULSE_SERVER` before launching babelmap there:
...
if babelmap were running locally.
...
All of them are ordinary Unix audio tools; babelmap doesn't know or care
...
sink babelmap plays to on the remote host, and pipe raw PCM over SSH to
...
the PulseAudio monitor of the sink babelmap uses, and listen on a client
