...
| `blorb` | Blorb container parsing — bundled story, cover art, and sound/image resources — plus the release-media readers beside it: Infocom's native picture archives, Amiga `.adf` floppies (`adf.rs`) and Macintosh DiskCopy 4.2 / HFS disks (`hfs.rs`). All hand-rolled; the crate takes no dependencies. `medium.rs` is the small piece on top: which machine a mounted image implies, and therefore which Z-machine interpreter number it defaults to. It lives here because `app` and `zvm-cli` share exactly one crate that recognises these filesystems — this one — and two copies of "an `.adf` means interpreter 4" is the policy that goes stale in one place and not the other (SQ-0839). An explicit number always outranks it. |
