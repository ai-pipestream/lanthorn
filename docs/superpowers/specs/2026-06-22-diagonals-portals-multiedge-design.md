...
- **Default placement (soft hint):** up/down are a *directional hint*, not an absolute placement. At discovery, an **Up** target prefers the cell **directly north** of its origin and a **Down** target **directly south**, but it **yields** (nearest free cell) instead of pushing rooms aside, and never overrides a compass placement (a room with compass edges is positioned by those; an already-placed room is never moved). In/Out/Unknown keep the nearest-free-cell placement. The `↑`/`↓` icon and dotted connector then render the relationship to wherever the room actually sits.
  - **Why a hint, not absolute:** a room reached by up/down often also has compass edges (e.g. an Attic that's `up` from the Kitchen but also has its own exits). Forcing it directly N/S — and pushing neighbours — fought the compass row/column chains and distorted those relationships. Making placement a soft hint that yields keeps the compass layout intact. (Extending up/down into the *relayout* constraint engine was tried and reverted: it broke the chain-alignment invariants.)
