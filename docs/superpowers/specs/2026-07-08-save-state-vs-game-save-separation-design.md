...
Cleanly separate lanthorn's two save mechanisms so they can never be confused — by
...
lanthorn has two distinct save mechanisms that today both write a `.lanthorn` archive
...
**Confirmed regression (SQ-0163):** an in-game `@save` writes a `.lanthorn` with
...
|---|---|---|---|---|
| **Save State** (Ctrl+S / auto-save / session resume) | `.lanthorn` | rich archive (map + screen + transcript + VM state) | resume point | resume (`restore_file`) |
...
Glulx has no `@save` opcode, so Glulx has **only** Save State (`.lanthorn`); there is no
Glulx game-save `.qzl`.

**The regression is fixed by construction:** no descriptor-PC data ever lands in a
`.lanthorn` again — `.lanthorn` archives are *always* resume-convention Save States, so
host restore of a `.lanthorn` always resumes correctly. Game saves are `.qzl` and always
complete the descriptor. The two can no longer be conflated because they are different
files.

### Save side

- **In-game `@save`** (`main.rs:4263`, currently `save_named` → `.lanthorn`): instead
  ...
  **no** map/screen/transcript — those live in the session `.lanthorn` (auto-save), and
...
- **Save State** (Ctrl+S / `save-state`, quit-save, exit-save, auto-save, named host
  slots — `main.rs:2445, 2484, 3798, 3893, 3910`): unchanged — `.lanthorn` archive,
...
- **`.lanthorn`** → resume (`restore_file`) — full Save State restore.
...
The saves manager keeps **one list** showing both `.lanthorn` and `.qzl` (it already
...
- **Red→green regression (zvm):** after an in-game `@save`, restoring the produced `.qzl`
  ...
  game seeing "restored"; and host restore of a `.lanthorn` still resumes. (The current
  behavior is red.)
- **Extension dispatch (app):** loading a `.qzl` completes the descriptor; loading a
  `.lanthorn` resumes; foreign `.qzl` import completes the descriptor.
...
- **Glulx game saves** — none exist (no `@save` opcode); Glulx keeps only `.lanthorn`.
...
- **Migration:** no backward compatibility — the user deletes old `.lanthorn` slots that
  were written with descriptor-PC (the post-SQ-0163, pre-SQ-0227 window). Legacy
  resume-convention `.lanthorn`s still restore correctly.
