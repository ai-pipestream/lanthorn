# Queued brief — Wire the `.babelmap` archive into save/load (finish TODO L7)
...
## Goal
Make the single-file `.babelmap` archive the primary persistence container (map + game save together), and make the legacy "shared default map" behavior optional via config. Today the map (`mapper`) and the game save (Quetzal) are written to SEPARATE files; this unifies them.
...
- **Archive path:** `<dir>/<ifid>.babelmap` (add `fn archive_path(base, ifid)` to `ifid.rs` mirroring `map_path`, or derive inline).
...
- **Load (startup, and `Ctrl+R` RestoreGame):** if `<ifid>.babelmap` exists → `archive::load_archive`, use `ac.mapper` and `session.machine.restore_quetzal(&ac.save)`. RestoreGame restores the game half from the archive.
...
- **Back-compat / migration:** if no `.babelmap` exists but a legacy `<ifid>.map.json` does, load that map (so existing users keep their maps); the next save writes the `.babelmap`. The game-save slot may stay as-is for back-compat, but new saves go in the archive.
...
Whether to STOP writing the legacy `.map.json` once `.babelmap` is the container, or keep both during a migration window. Default: keep writing `.map.json` only when `use_default_map = true`; otherwise the archive is the sole container. Confirm at review.
